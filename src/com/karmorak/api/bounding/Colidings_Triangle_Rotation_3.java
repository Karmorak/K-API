...
//		} else return -1;
		float x = (b_pos_x + (b_width-b_width * (t_y / b_height)));
		return x;		
	}
...
//		} else return -1;		
		float y = b_pos_y + (b_height - b_height * (t_x / b_width));	
		return y;
	}
...
//		} 	
	...
	static void colides_Right_Side_t(Colideable p, Colideable tri, float move) {
		...
		if(a_pos_x < b_pos_x+1 && a_pos_x >= b_pos_x) {
			if(a_pos_y > b_pos_y+b_height-a_height && a_pos_y < b_pos_y + b_height) {
				a_pos_x = b_pos_x;
			}
		}
		p.setPosition(a_pos_x - a_width, a_pos_y - a_height);
...
//		} 	
		...
		p.setPosition(a_pos_x - a_minwidth, a_pos_y - a_minwidth);				
